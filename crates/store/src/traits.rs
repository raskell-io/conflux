...
/// An async storage backend for Conflux.
///
/// Designed for backends requiring async I/O (PostgreSQL, DynamoDB).
/// Mirrors the synchronous [`Store`] trait with async methods.
///
/// Implementors must be `Send + Sync` for thread-safe usage in async contexts.
///
/// # Built-in Implementations
///
/// - [`PostgresStore`](crate::PostgresStore) — PostgreSQL backend (feature: `postgres`)
/// - [`DynamoStore`](crate::DynamoStore) — DynamoDB backend (feature: `dynamodb`)
#[cfg(any(feature = "postgres", feature = "dynamodb"))]
#[async_trait]
